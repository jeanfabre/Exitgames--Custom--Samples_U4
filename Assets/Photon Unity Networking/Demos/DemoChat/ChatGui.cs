...
public class ChatGui : MonoBehaviour, IChatClientListener
{
	...
	private readonly Dictionary<string,FriendsItem> friendListItemLUT = new Dictionary<string, FriendsItem>();
   ...
   // private static string WelcomeText = "Welcome to chat. Type \\help to list commands.";
    ...
    private void SendChatMessage(string inputLine)
    {
        ...
        if (inputLine[0].Equals('\\'))
        {
            ...
            if (tokens[0].Equals("\\state"))
            {
				...
				int newState = int.Parse(subtokens[0]);

				List<string> _messages = new List<string>();
				_messages.Add("i am state " + newState);
				if (subtokens.Length >= 2)
				{
					_messages.Add(subtokens[1]);
				}

				this.chatClient.SetOnlineStatus(newState, _messages.ToArray()); // this is how you set your own state and (any) message
    ...
    public void OnConnected()
    {
		...
		if (FriendsList!=null  && FriendsList.Length>0)
		{
			...
			if (this.FriendListUiItemtoInstantiate != null )
			{
				...
				GameObject.Destroy(this.FriendListUiItemtoInstantiate);
			}
    ...
    public void OnStatusUpdate(string user, int status, bool gotMessage, object message)
    {
		...
		if (friendListItemLUT.ContainsKey(user) && friendListItemLUT[user] !=null) 
	    {
			friendListItemLUT[user].OnFriendStatusUpdate(status,gotMessage,message);
		}
